...
# Setup logging - use INFO in production for security
log_level = logging.DEBUG if os.environ.get('FLASK_ENV') == 'development' else logging.INFO
logging.basicConfig(level=log_level)

# Configure Gemini API
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "AIzaSyDVvBockK-W45GKypKz7JuxxFSVeSdE6w4")
...
if GEMINI_API_KEY and len(GEMINI_API_KEY) > 10:  # Basic validation
    ...
    if not GEMINI_API_KEY:
        logging.warning("GEMINI_API_KEY not configured - AI features disabled")
...
if not session_secret:
    # Allow fallback only in development/Replit environment
    if os.environ.get('FLASK_ENV') == 'development' or os.environ.get('REPLIT_DEV_DOMAIN'):
        session_secret = "fallback_secret_key_for_development_only"
        logging.warning("Using fallback session secret - set SESSION_SECRET for production")
    else:
        raise ValueError("SESSION_SECRET environment variable must be set in production")
app.secret_key = session_secret
app.wsgi_app = ProxyFix(app.wsgi_app, x_proto=1, x_host=1)

# Configure cache control to prevent caching issues in preview
@app.after_request
...
def resposta_inimigo():
    try:
        ...
        if model:
            ...
            ai_response = "IA temporariamente indisponível. Configure GEMINI_API_KEY."
...
def chat_send():
    try:
        ...
        context = """
        Você é o "Rei do Mate", assistente oficial do site "In the Shadows" - Laboratório de Estratégias de Xadrez.
        
        SOBRE O SITE E CRIADOR:
        - Site criado por xKaike (nick no chess.com)
        - xKaike é o criador original da estratégia "In the Shadows"
        - Este é um laboratório pessoal para desenvolver e documentar estratégias de xadrez
        - Site tem seções: Início, Teoria, Exemplos, Conceitos, Fundamentos, Exercícios, Reflexões, Chat
        - Foco na metodologia "In the Shadows": usar execução parcial de movimentos conhecidos para criar decepção, posicionando uma peça oculta para o golpe decisivo
        
        FUNCIONALIDADES DO SITE:
        - Exercícios táticos interativos (iniciante até ranking 5000)
        - Análise de conceitos estratégicos
        - Documentação de teorias e exemplos
        - Sistema de resolução de exercícios com pontuação
        - Chat com IA especializada (você)
        
        REGRAS DE RESPOSTA:
        - Responda em 2-3 frases curtas e claras
        - Seja direto mas amigável
        - Use português brasileiro natural
        - Para cumprimentos (oi, opa, olá), responda de forma calorosa: "Oi! Como posso ajudar com xadrez?"
        - Para perguntas técnicas, seja preciso e educativo
        - Se perguntarem sobre o site ou xKaike, responda com conhecimento completo
        ...
        if model:
            # Configurar geração mais rápida
            generation_config = {
                "temperature": 0.7,
                "top_p": 0.8,
                "top_k": 40,
                "max_output_tokens": 150,  # Limitar tokens para respostas mais rápidas
            }
